...
_last_frame_used_path: str | None = None
...
_SELECTED_EXT: str | None = None  # ".jpg" | ".png" | ".bmp"


def _try_encode(ext: str, img, params) -> bytes | None:
    try:
    ...
    try:
        publish(
            "vision.detections",
            {
                "size": [int(w), int(h)],
                "items": [
                    {"name": name, "score": float(conf), "bbox": [int(x1), int(y1), int(x2 - x1), int(y2 - y1)]}
...
def draw_overlay(img, detections):
    for name, conf, (x1, y1, x2, y2) in detections:
        ...
        cv2.putText(img, f"{name}:{conf:.2f}", (x1, max(0, y1 - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
...
def main():
    ...
    print(
        f"[preview] Start. LCD={'ON' if (LCD_ok and not ENV_NO_DRAW) else 'OFF (headless)'}; "
        f"ROT={ROT}°; SAVE_LAST_EVERY={SAVE_LAST_EVERY}; DETECTOR={det_kind}; PUB={PUB_kind}; "
        f"LAST_BASE={LAST_BASE} (auto ext); SNAP_DIR={SNAP_DIR}",
        flush=True,
    )
