...
def draw_face(canvas, cfg, model, guide=True, quality="fast"):
    ...
    l = (cx - eye_dx - eye_w // 2, cy - int(eye_h * blink_mul), cx - eye_dx + eye_w // 2, cy + int(eye_h * blink_mul))  # noqa: E741
    ...
    brow_w = int(S * 0.19)
    brow_h = int(S * getattr(cfg, "brow_h_k", 0.09))
    ...
    def mouth_curvature_for(state: str) -> float:
        k = {"idle": -0.48, "wake": -0.36, "record": -0.28, "process": -0.22, "low_battery": 0.25, "speak": -0.18}.get(
            state, -0.24
        )
    ...
    if getattr(model, "assist_speaking", False) or getattr(model, "state", "") == "speak":
        ...
        canvas.rectangle(
            [(cx - width // 2, mouth_y - height // 2), (cx + width // 2, mouth_y + height // 2)], fill=(0, 0, 0)
        )
    else:
