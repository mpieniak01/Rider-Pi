...
import os, sys, time, signal, pathlib, importlib.util
...
def img_to_rgb565(img: Image.Image) -> bytes:
    im = img.convert("RGB")
    w,h = im.size
    out = bytearray(w*h*2); i=0; px = im.load()
    for y in range(h):
        for x in range(w):
            v = rgb565(*px[x,y])
            out[i]   = (v >> 8) & 0xFF
...
def _load_face_api_module() -> Optional[object]:
    """
...
def main():
    ...
    last = time.time(); n=0; last_sink = ""
    print(f"[face] backend={backend} fb={fbdev} size={size} fb_size={fb_w}x{fb_h} rotate={rotate} expr={expr} use_api={use_api}")
    while running:
        # 1) render buźki
        png = fc.frame()
        ...
        # 4) obrót
        if rotate: canvas = canvas.rotate(rotate, expand=True)
