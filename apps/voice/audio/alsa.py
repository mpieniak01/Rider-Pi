...
def probe_devices() -> dict[str, Any]:
    ...
    try:
        ...
        logger.event("alsa.probe.success", 
                ...
                if len(parts) >= 2:
                    ...
                    if any(cmd in command.lower() for cmd in ["arecord", "aplay", "apps.voice.cli"]):
                        try:
                            ...
                            check = subprocess.run(["kill", "-0", pid], 
...
def ensure_free(capture_dev: str | None = None, playback_dev: str | None = None, 
    ...
    logger.event("alsa.ensure_free.start", 
    ...
    # Log final status
    logger.event("alsa.ensure_free.complete",
