...
from apps.ui.face_renderers import BaseRenderer, FaceConfig, LCDRenderer, TKRenderer
...
class FaceApp:
    ...
    def _maybe_print_bench(self):
        ...
        if now_wall - self._bench_last_wall >= 1.0:
            ...
            print(
                f"[bench] fps={fps:.1f}  cpu~{cpu_pct:.0f}%  draw={draw_ms:.1f}ms  push={push_ms:.1f}ms{extra}",
                flush=True,
            )
    ...
    def _maybe_heartbeat(self):
        ...
        try:
            ...
            payload = {
                ...
                "backend": (
                    "lcd"
                    if isinstance(self.renderer, LCDRenderer)
