...
class LCDRenderer(BaseRenderer):
    ...
    # --- LCD helpers ---
    def _apply_spi_hz(self, hz: int | None):
        if hz is None:
            return
        try:
...
class TKRenderer(BaseRenderer):
    ...
    def render(self, model: Any):
        ...
        if speaking:
            ...
            k = {"idle": -0.48, "wake": -0.36, "record": -0.28, "process": -0.22, "low_battery": 0.25}.get(
                model.state, -0.24
            )
            if (getattr(model, "expr", None) or "") == "happy":
                k -= 0.18 * max(0.0, min(1.0, float(getattr(model, "expr_intensity", 0.0))))
            ...
            self.canvas.itemconfig(
                self.mouth_arc, start=start, extent=extent, width=max(8, int(S * 0.055)), state="normal"
            )
