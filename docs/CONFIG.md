...
- Now loads from `apps.motion.config.load_motion_bridge_config()` in `services/motion_bridge.py` (legacy path); now superseded by `motion-executor` / `sensor-reader` which share the same config file.
