...
- Jeden punkt wejścia: **Python** (`scripts/sys_splash-info.py`) – usunięte duplikaty `.sh`.
...
- `systemd/rider-post-splash.service` → pojedynczy `ExecStart`:
  `SPLASH_WAIT_IP_S=60 /usr/bin/python3 scripts/sys_splash-info.py`
- `systemd/rider-boot-prepare.service` → `SPLASH_SECONDS=3`
- Usunięte: `scripts/sys_splash-info.sh`, `ops/net_wait_ip.sh` (zastąpione logiką w Pythonie).
...
- `SPLASH_WAIT_IP_S=10 python3 scripts/sys_splash-info.py` (powinno pokazać IP i zapisać log).
