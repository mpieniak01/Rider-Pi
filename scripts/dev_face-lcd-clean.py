...
def _pick_lcd_class(fr, prefer=None):
    if prefer:
        for c in _list_lcd_classes(fr):
            if c.__name__ == prefer:
                return c
        raise AttributeError(
            f"Nie znaleziono klasy '{prefer}'. Dostępne: {[c.__name__ for c in _list_lcd_classes(fr)]}"
        )
...
def _build_lcd(fr, drv_rotate, prefer=None):
    ...
    try:
        if hasattr(dev, "SPI") and spi_hz:
            dev.SPI.max_speed_hz = spi_hz
    except Exception:
        pass
    # wymiary
