...
def should_skip_context(line: str, context_before: List[str], filepath: Path) -> bool:
    ...
    if line.strip().startswith('|') and '|' in line:
        ...
        for prev_line in context_before[-10:]:
            if '|' in prev_line and any(header in prev_line for header in 
                ['Old Path', 'New Path', 'Źródło', 'Cel', 'Source', 'Target', 
                 'Before', 'After', 'Was', 'Now']):
                return True
    ...
    if any(marker in line.lower() for marker in 
           ['# old:', '# before:', 'old path', 'removed in pr', 'was a stub',
            ...
            if old_ref.startswith(r'python3') or '\\' in old_ref:
                ...
                if pattern.search(line):
                    ...
                    if verify_new_path_exists(new_ref):
                        ...
                        print(f"WARNING: {filepath.relative_to(REPO_ROOT)} line {i+1}: "
                              f"Target {new_ref} not found, skipping update", file=sys.stderr)
            else:
                ...
                if old_ref in line:
                    ...
                    if verify_new_path_exists(new_ref):
                        ...
                        print(f"WARNING: {filepath.relative_to(REPO_ROOT)} line {i+1}: "
                              f"File {new_ref} not found, skipping update", file=sys.stderr)
...
def main():
    ...
    parser.add_argument('--verify-only', action='store_true',
                        help='Only verify that new paths exist')
