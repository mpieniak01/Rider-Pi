...
def _r_camera_raw():
    return core.camera_raw()


@app.route("/camera/proc", methods=["GET", "HEAD"])
def _r_camera_proc():
    return core.camera_proc()


@app.route("/camera/last", methods=["GET", "HEAD"])
def _r_camera_last():
    return core.camera_last()


@app.route("/camera/placeholder", methods=["GET", "HEAD"])
def _r_camera_placeholder():
    return core.camera_placeholder()


@app.route("/snapshots/<path:fname>")
def _r_snapshots(fname):
    return core.snapshots_static(fname)


# Services
@app.route("/svc", methods=["GET"])
def _r_svc_list():
    return core.svc_list()


@app.route("/svc/<name>/status", methods=["GET"])
def _r_svc_status(name):
    return core.svc_status(name)


@app.route("/svc/<name>", methods=["POST"])
def _r_svc_action(name):
    return core.svc_action(name)
...
def _r_api_move():
    return core.api_move()


@app.route("/api/stop", methods=["POST"])
def _r_api_stop():
    return core.api_stop()


@app.route("/api/preset", methods=["POST"])
def _r_api_preset():
    return core.api_preset()


@app.route("/api/voice", methods=["POST"])
def _r_api_voice():
    return core.api_voice()
...
def control_post_relay():
    ...
    if "Content-Type" in hdrs:
        out.headers["Content-Type"] = hdrs["Content-Type"]
    return out
