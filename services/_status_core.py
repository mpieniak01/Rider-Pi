...
# --- API compatibility: /api/control (GET/POST) ------------------------------
def api_control():
    # przyjmij POST JSON lub GET query
    data = request.get_json(silent=True) or {}
    ...
    # mapowanie akcji na wektory prędkości (proste)
    # vx: przód(+)/tył(-), yaw: obrót prawo(+)/lewo(-)
    vx = 0.0; yaw = 0.0
    ...
    elif action in ("stop", "halt"):  # STOP
        bus_pub("cmd.stop", {"ts": time.time()})
        return Response(json.dumps({"ok": True, "sent": {"action": "stop"}}), mimetype="application/json")

    # gdy akcja nieznana → 400
    if vx == 0.0 and yaw == 0.0 and action not in ("stop",):
        return Response(json.dumps({"ok": False, "error": "unknown action"}), mimetype="application/json", status=400)

    # wyślij ruch
    bus_pub("cmd.move", {"vx": vx, "yaw": yaw, "duration": ms/1000.0, "ts": time.time()})
