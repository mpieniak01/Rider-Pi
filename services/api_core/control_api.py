...
def api_cmd():
    ...
    try:
        ...
        return Response(
            '{"ok": true, "note": "unknown type -> cmd.raw"}', mimetype="application/json"
        )
...
def api_control():
    ...
    if vx == 0.0 and yaw == 0.0 and action not in ("stop",):
        return Response(
            '{"ok": false, "error": "unknown action"}', mimetype="application/json", status=400
        )
