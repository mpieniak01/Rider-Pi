...
def _safe_surface(size, with_alpha: bool = False) -> pygame.Surface:
    """Safe pygame.Surface: int(w, h) + 32bpp when alpha is requested."""
    w, h = size
    w, h = int(w), int(h)
    if with_alpha:
        return pygame.Surface((w, h), flags=pygame.SRCALPHA, depth=32)
...
class World:
    ...
    def render_main_panel(self, robot=None):
        ...
        if robot:
            ...
            # Rotate robot
            robot_surf = _safe_surface((robot_size * 2, robot_size * 2), with_alpha=True)
