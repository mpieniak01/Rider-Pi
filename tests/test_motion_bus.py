...
def main():
    ...
    sub.setsockopt_string(zmq.SUBSCRIBE, ECHO_TOPIC)
    ...
    print("\n[TEST] Naprzód 0.3s")
    send({"type": "drive", "lx": 0.15, "az": 0.0})
    time.sleep(0.3)
    send({"type": "stop"})
    wait_echo()
    ...
    send({"type": "drive", "lx": 0.0, "az": -0.35})
    time.sleep(0.5)
    send({"type": "stop"})
    wait_echo()
    ...
    send({"type": "spin", "dir": "left", "speed": 0.4, "dur": 0.5})
    wait_echo(1200)

    print("\n[TEST] Spin RIGHT 0.5s (komenda 'spin' jeśli wspierana)")
    send({"type": "spin", "dir": "right", "speed": 0.4, "dur": 0.5})
    wait_echo(1200)
