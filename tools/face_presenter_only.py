#!/usr/bin/env python3
import os, sys, time, importlib, pkgutil, argparse
...
def make_testcard(w,h):
    ...
    for i,c in enumerate(colors): d.rectangle((0, i*bh, w, (i+1)*bh-1), fill=c)
    d.rectangle((0,0,w-1,h-1), outline="white", width=3)
    d.line((0,0,w-1,h-1), fill="white", width=3); d.line((0,h-1,w-1,0), fill="white", width=3)
    return img
...
def main():
    ...
    if spi is not None:
        try:
            if hz: spi.max_speed_hz = hz
            if hasattr(spi, "mode"): spi.mode = mode
        except Exception: pass
    ...
    print(f"[presenter] frames={frames} time={time.time()-t0:.2f}s fps~{frames/max(0.001, (time.time()-t0)):.2f} W={W} H={H} hz={hz} mode={mode}")
