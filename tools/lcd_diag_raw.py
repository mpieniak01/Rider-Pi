#!/usr/bin/env python3
import os, sys, time, struct, importlib, inspect, pkgutil
...
def iter_xgo_modules():
    try:
        ...
        print("[diag] FATAL: brak xgoscreen:", e); sys.exit(2)
    mods=[xgoscreen]
    try:
    ...
    for m in iter_xgo_modules():
        for name, obj in vars(m).items():
            if inspect.isclass(obj) and obj.__module__ == m.__name__:
                has_show = any(callable(getattr(obj, k, None)) for k in ("ShowImage","show_image","display","blit","put","present"))
                has_raw  = all(hasattr(obj, k) for k in ("SetWindows","command","spi_writebyte"))
                if has_raw:
                    cands.append((name,obj))
    if not cands:
        print("[diag] FATAL: brak klasy z SetWindows/command/spi_writebyte w xgoscreen.*"); sys.exit(2)
    # preferuj nazwy z '2inch' lub 'lcd'
    cands.sort(key=lambda kv: (0 if "2inch" in kv[0].lower() else 1,
...
def main():
    ...
    if spi is not None:
        try:
            if SPIHZ: spi.max_speed_hz = SPIHZ
            if hasattr(spi, "mode"): spi.mode = SPIMODE
    ...
    print("[diag] RED")
    fill565(0xF800); time.sleep(0.2)
    print("[diag] GREEN")
    fill565(0x07E0); time.sleep(0.2)
    print("[diag] BLUE")
    fill565(0x001F); time.sleep(0.2)
    print("[diag] DONE")
