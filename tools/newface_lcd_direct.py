...
NAME_OK = re.compile(
    r"(img|image|frame|png|rgb|buf|buffer|disp|show|blit|push|draw|render|present|send|write|update|put)", re.I
)
...
class LCDDirect:
    def __init__(self, rotate: int, size: int, spi_hz: int | None = None, bl_pin: int = 13, force: str | None = None):
        self.rotate = rotate
        self.size = size
        self.spi_hz = spi_hz
        self.bl_pin = bl_pin
    ...
    def _init_lcd(self):
        ...
        if need_cfg:
            ...
            try:
                self._lcd = cls(
                    lcd_do_init=True, lcd_rotate=self.rotate, lcd_spi_hz=self.spi_hz, lcd_bl_pin=self.bl_pin
                )
            except TypeError:
                try:
...
def main():
    ...
    ap.add_argument(
        "--secs", type=float, default=None, help="Czas trwania testu w sekundach (domyślnie: nieskończoność)"
    )
    args = ap.parse_args()
