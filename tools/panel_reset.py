#!/usr/bin/env python3
import os, sys, time, importlib, struct
...
W = getattr(lcd,'width',240); H = getattr(lcd,'height',320)
...
print(f"Panel reinitialized (COLMOD=0x{COLMOD:02X}, MADCTL=0x{MADCTL:02X}, invert={'on' if INVERT else 'off'}, SPI={SPIHZ or getattr(spi,'max_speed_hz','-')} mode={getattr(spi,'mode','-')}).")
