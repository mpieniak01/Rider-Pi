#!/usr/bin/env python3
import os, sys, time, argparse, importlib, pkgutil, inspect, pathlib
    ...
    for m in iter_xgo_modules():
        for n,o in vars(m).items():
            if inspect.isclass(o) and o.__module__==m.__name__:
                has_show = any(callable(getattr(o,a,None)) for a in ("ShowImage","show_image","display","blit","put","present"))
...
def main():
    ...
    for p in os.getenv("RIDER_APPS_PATH","apps").split(":"):
        ...
        if p:
            cand=(ROOT/p).resolve()
            if cand.exists(): sys.path.insert(0,str(cand))

    Dev=pick_device_class()
    try: dev=Dev()
    except TypeError: dev=Dev
    ...
    spi_now=0
    try: spi_now = getattr(getattr(dev,"SPI",None),"max_speed_hz",0) or 0
    except Exception: pass
    print(f"Panel reinitialized (COLMOD=0x{int(str(args.colmod),0):02X}, "
